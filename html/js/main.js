$(function() {
    ...
    var loadFeed = function(file) {
        $.ajax({
            ...
            success: function(response) {
                ...
                if ($(".feed ul .tweet-entity").anchorTextUrls(), $(window).width() < 800 ? $(".feed img").lazyload() : $(".feed img").lazyload({
                    container: $(".feed ul")
                }), $(window).width() < 800) {
                    $(".feed > ul").slick({
                        slide: "li",
                    ...
                    $(".feed > ul").on("swipe", function(event, slick, direction) {
                        ga("send", "event", "social feed", "swipe: " + swipeCounter), swipeCounter++;
                    });
                }
            }
        });
    ...
    var twitterTemplate = function(profileImage, twitterName, twitterUser, twitterTime, twitterTweetUrl, twitterTweetEntity, twitterImage) {
        ...
        twitterDisplayImage = "", null !== twitterImage && (twitterDisplayImage = '<img data-original="' + twitterImage + '" src="/img/preloader-larger.gif" alt="">');
        var twitterMarkup = "";
        ...
        twitterMarkup += "        </div>", twitterMarkup += "    </div>", twitterMarkup += '    <div class="actions">', 
        twitterMarkup += "        <ul>", twitterMarkup += "            <li>", twitterMarkup += '                <a href="' + intentReply + '">', 
